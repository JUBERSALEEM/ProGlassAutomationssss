    ...
    namespace ProGlassAutomation.ViewModels
    {
        ...
        // PATCH E1: Recalculation gate to prevent recursive calls
        private bool _isRecalculating = false;

        // PATCH B2: Prevent duplicate event firing for charges
        ...
        private void Item_PropertyChanged(object? sender, PropertyChangedEventArgs e)
        {
            ...
            {
                // PATCH A2: Only recalculate spec level
                // The spec's PropertyChanged will trigger Spec_PropertyChanged
                // which already calls Invoice.CalculateTotals() - no need to call twice
                item.Specification.Recalculate();
                Invoice.IsDirty = true;

                // REMOVED: SafeRecalculateInvoice(); // This was causing dual cascade!
                // The event chain is now:
                // Item changes → Spec.Recalculate() → Spec fires PropertyChanged 
                // → ViewModel Spec_PropertyChanged → Invoice.CalculateTotals()
            }
        }

        // REMOVED: SafeRecalculateInvoice() method - not needed anymore
