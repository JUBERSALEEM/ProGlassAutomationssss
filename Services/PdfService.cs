    ...
    public static class PdfService
    {
        public static void PrintProformaInvoice(ProformaInvoiceModel invoice)
        {
            ...
            printDoc.PrintPage += (sender, e) =>
            {
                ...
                e.Graphics.DrawString("PROFORMA INVOICE", boldFont, Brushes.Black, pageWidth / 2 - 80, y);
                y += 30;

                e.Graphics.DrawString($"Invoice No: {invoice.InvoiceNo}", font, Brushes.Black, 50, y);
                e.Graphics.DrawString($"Date: {invoice.InvoiceDate:yyyy-MM-dd}", font, Brushes.Black, pageWidth - 200, y);
                y += 25;
                e.Graphics.DrawString($"Valid Until: {invoice.ValidUntil:yyyy-MM-dd}", font, Brushes.Black, 50, y);
                y += 30;

                e.Graphics.DrawString("From:", boldFont, Brushes.Black, 50, y);
                y += 20;
                e.Graphics.DrawString(invoice.CompanyName, font, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"TRN: {invoice.CompanyTRN}", smallFont, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString(invoice.CompanyLocation, smallFont, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"Phone: {invoice.CompanyPhone}", smallFont, Brushes.Black, 50, y); y += 30;

                e.Graphics.DrawString("Bill To:", boldFont, Brushes.Black, 50, y);
                y += 20;
                e.Graphics.DrawString(invoice.CustomerName, font, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"TRN: {invoice.CustomerTRN}", smallFont, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString(invoice.CustomerAddress, smallFont, Brushes.Black, 50, y); y += 30;

                e.Graphics.DrawString($"Project: {invoice.ProjectName}", font, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"Location: {invoice.ProjectLocation}", smallFont, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"LPO No: {invoice.LPONo}", smallFont, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"Attention: {invoice.AttentionName} | Contact: {invoice.ContactNo}", smallFont, Brushes.Black, 50, y); y += 30;
                ...
                e.Graphics.DrawString("Width", font, Brushes.Black, 300, y + 3);
                e.Graphics.DrawString("Height", font, Brushes.Black, 370, y + 3);
                e.Graphics.DrawString("Qty", font, Brushes.Black, 440, y + 3);
                e.Graphics.DrawString("SQM", font, Brushes.Black, 480, y + 3);
                e.Graphics.DrawString("Price", font, Brushes.Black, 540, y + 3);
                e.Graphics.DrawString("Total", font, Brushes.Black, 620, y + 3);
                y += 25;

                foreach (var piItem in invoice.Items)
                {
                    e.Graphics.DrawString(piItem.SrNo.ToString(), smallFont, Brushes.Black, 55, y);
                    e.Graphics.DrawString(piItem.GlassRef, smallFont, Brushes.Black, 90, y);
                    e.Graphics.DrawString($"{piItem.Width1:F0}x{piItem.Height1:F0}", smallFont, Brushes.Black, 300, y);
                    e.Graphics.DrawString($"{piItem.Width2:F0}x{piItem.Height2:F0}", smallFont, Brushes.Black, 370, y);
                    e.Graphics.DrawString(piItem.Qty.ToString(), smallFont, Brushes.Black, 440, y);
                    e.Graphics.DrawString($"{piItem.TotalSQM:F2}", smallFont, Brushes.Black, 480, y);
                    e.Graphics.DrawString($"{piItem.Price:F2}", smallFont, Brushes.Black, 540, y);
                    e.Graphics.DrawString($"{piItem.TotalPrice:F2}", smallFont, Brushes.Black, 620, y);
                    y += 18;
                }

                y += 20;
                e.Graphics.DrawString($"Sub Total: AED {invoice.TotalAmount:F2}", font, Brushes.Black, 500, y); y += 20;
                e.Graphics.DrawString($"VAT (5%): AED {invoice.VATAmount:F2}", font, Brushes.Black, 500, y); y += 20;
                e.Graphics.DrawString($"NET AMOUNT: AED {invoice.NetAmount:F2}", boldFont, Brushes.Black, 500, y); y += 30;
                ...
                e.Graphics.DrawString("Thank you for your business!", smallFont, Brushes.Gray, 50, y);
        ...
        public static void PrintDeliveryOrder(DeliveryOrderModel d)
        {
            ...
            printDoc.PrintPage += (sender, e) =>
            {
                ...
                e.Graphics.DrawString("DELIVERY ORDER", boldFont, Brushes.Black, pageWidth / 2 - 70, y);
                y += 30;
                ...
                y += 20;
                e.Graphics.DrawString(d.ClientName, font, Brushes.Black, 50, y); y += 15;
                e.Graphics.DrawString($"Project: {d.ProjectName}", smallFont, Brushes.Black, 50, y); y += 30;

                e.Graphics.DrawString($"Vehicle: {d.VehicleNumber}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Driver: {d.DriverName}", font, Brushes.Black, 50, y); y += 30;
                ...
                foreach (var doItem in d.Items)
                {
                    ...
                    e.Graphics.DrawString(doItem.GlassRef, smallFont, Brushes.Black, 90, y);
                ...
                e.Graphics.DrawString($"Status: {d.Status}", boldFont, Brushes.Black, 50, y);

                if (!string.IsNullOrWhiteSpace(d.Notes))
                {
                    y += 25;
                    e.Graphics.DrawString("Notes:", boldFont, Brushes.Black, 50, y); y += 20;
                    e.Graphics.DrawString(d.Notes, smallFont, Brushes.Black, 50, y);
                }
        ...
        public static void PrintDailyWork(DailyWork w)
        {
            ...
            printDoc.PrintPage += (sender, e) =>
            {
                ...
                e.Graphics.DrawString("DAILY WORK REPORT", boldFont, Brushes.Black, pageWidth / 2 - 80, y);
                y += 30;

                e.Graphics.DrawString($"Date: {w.Date:yyyy-MM-dd}", font, Brushes.Black, 50, y);
                e.Graphics.DrawString($"Company: {w.Company}", font, Brushes.Black, pageWidth - 250, y);
                y += 30;

                e.Graphics.DrawString($"PI Number: {w.PINumber}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Customer Ref: {w.CustomerReference}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Type of Work: {w.TypeOfWork}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Production Status: {w.ProductionStatus}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Salesman: {w.Salesman}", font, Brushes.Black, 50, y); y += 20;
                e.Graphics.DrawString($"Color: {w.Color}", font, Brushes.Black, 50, y); y += 30;

                e.Graphics.FillRectangle(Brushes.LightGray, 50, y, pageWidth - 100, 20);
                e.Graphics.DrawString("Qty", font, Brushes.Black, 55, y + 3);
                e.Graphics.DrawString("SQM", font, Brushes.Black, 150, y + 3);
                e.Graphics.DrawString("Status", font, Brushes.Black, 250, y + 3);
                e.Graphics.DrawString("Daily Report", font, Brushes.Black, 400, y + 3);
                y += 25;

                e.Graphics.DrawString(w.Qty.ToString(), smallFont, Brushes.Black, 55, y);
                e.Graphics.DrawString($"{w.SQM:F2}", smallFont, Brushes.Black, 150, y);
                e.Graphics.DrawString(w.Status, smallFont, Brushes.Black, 250, y);
                e.Graphics.DrawString(w.DailyReportStatus, smallFont, Brushes.Black, 400, y);
                y += 30;
