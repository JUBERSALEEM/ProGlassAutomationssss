...
VALUES ($cat, $th, $col, $hex, $w, $h, $sqm, $pp, $sp, $ts, $us, $bs, $act, $sup, $supn, $desc, $cd, $lpd)";
        ...
        public static void SaveProformaInvoice(ProformaInvoiceModel pi)
        {
            Execute(conn =>
            {
                ...
                if (existingId != null)
                {
                    ...
                    cmd.Parameters.AddWithValue("$pd", pi.InvoiceDate.ToString("yyyy-MM-dd"));
                    cmd.Parameters.AddWithValue("$vu", pi.ValidUntil.ToString("yyyy-MM-dd"));
        ...
        public static void SaveJobOrder(JobOrderModel jo)
        {
            ...
            if (existingId != null)
            {
                ...
                using var cmd = conn.CreateCommand();
                cmd.CommandText = @"UPDATE JobOrders SET ProformaInvoiceId = $piId, ClientName = $cn, ProjectName = $pn, ProjectLocation = $pl, JODate = $jd, RequiredDate = $rd, Status = $st, TotalQty = $tq, ReleasedQty = $rq, BalanceQty = $bq, TotalAmount = $ta, Notes = $nt, UpdatedDate = $ud WHERE Id = $id";
                ...
                cmd.Parameters.AddWithValue("$jd", jo.JODate.ToString("yyyy-MM-dd"));
                cmd.Parameters.AddWithValue("$rd", jo.RequiredDate.ToString("yyyy-MM-dd"));
            ...
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = @"INSERT INTO JobOrders (JONumber, ProformaInvoiceId, ClientName, ProjectName, ProjectLocation, JODate, RequiredDate, Status, TotalQty, ReleasedQty, BalanceQty, TotalAmount, Notes, CreatedDate, UpdatedDate)
                    VALUES ($jo, $piId, $cn, $pn, $pl, $jd, $rd, $st, $tq, $rq, $bq, $ta, $nt, $cd, $ud)";
        ...
        public static List<JobOrderModel> GetAllJobOrders()
        {
            return Execute(conn =>
            {
                ...
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "SELECT * FROM JobOrders ORDER BY Id DESC";
                using var r = cmd.ExecuteReader();
                while (r.Read())
                {
        ...
        public static void DeleteJobOrder(int id)
        {
            ...
            {
                using var cmd = conn.CreateCommand();
                cmd.CommandText = "DELETE FROM JobOrderItems WHERE JobOrderId = $id";
                cmd.Parameters.AddWithValue("$id", id);
                cmd.ExecuteNonQuery();
        ...
        public static void SaveDeliveryOrder(DeliveryOrderModel d)
        {
            Execute(conn =>
            {
                ...
                if (existingId != null)
                {
                    ...
                    cmd.Parameters.AddWithValue("$dd", d.DODate.ToString("yyyy-MM-dd"));
        ...
        public static void SaveTaxInvoice(TaxInvoiceModel ti)
        {
            ...
            if (existingId != null)
            {
                ...
                cmd.Parameters.AddWithValue("$idate", ti.InvoiceDate.ToString("yyyy-MM-dd"));
                cmd.Parameters.AddWithValue("$dd", ti.DueDate.ToString("yyyy-MM-dd"));
